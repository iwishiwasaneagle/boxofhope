...
show_usage()
{
    std::cerr << "Welcome to " << IO_server_PROJECT_NAME << " (v"
              << IO_server_VERSION_MAJOR << "." << IO_server_VERSION_MINOR
              << "." << IO_server_VERSION_PATCH << ")" << std::endl
              << std::endl
              << "Usage:" << std::endl
              ...
              << "   -U,--user  <ip>          Home device - the device used to "
                 "check if the user is home"
              << std::endl;
}
...
main(int argc, char* argv[])
{
    ...
    while (1) {
        ...
        // Parse options. Note the flags are the same as bash getopts (probably
        // not a coincidence)
...
/*! Test function to serve as an example until the team is more up to speed with
 * C++, Doxygen, etc.
 *
 * \return Exit code
 */
...
tests()
{
    ...
    for (int i = 0; i < 10; i++) {
        std::cout << "This is running in the main thread! ("
                  << boost::this_thread::get_id() << ")" << std::endl;
        sleep(1);
    }

    std::cout << "Stopping IsUserHome runnable from main thread. ("
              << boost::this_thread::get_id() << ")" << std::endl;
