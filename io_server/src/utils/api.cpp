...
json API::operation(std::string method, std::string endpoint, json body, json headers){
	CURL *curl;
	CURLcode res;
	curl = curl_easy_init();
	...
	if(curl) {
        ...
        if(!PROXY_URL.empty()){
            ...
            std::cout << "\033[1;45mAPI Util\033[0m\tProxy set to " << PROXY_URL << " | Res = " << res << " | " << curl_easy_strerror(res) << std::endl;
        }
        ...
        std::cout << "\033[1;45mAPI Util\033[0m\tCall to " << path << " | Res = " << res << " | " << curl_easy_strerror(res) << std::endl;
        if(res!=CURLE_OK){
            return NULL;
        }

        // std::string body_delimiter = "\r\n\r\n";
        // responseBuffer = responseBuffer.erase(0,responseBuffer.find(body_delimiter)+body_delimiter.length()); // This was working but now it doesn't... seems like libcurl has started cutting the reponse down???
	}
