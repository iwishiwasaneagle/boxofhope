...
bool io::IsUserHome_Runnable::isUserHome(std::string ip) {
    ...
    FILE *f = popen((cmd + ip).c_str(),
                    "r"); //< Uses `ping` for now, move towards nmap -sP with
                          // sudo to detect mac addresses
...
io::IsUserHome_Runnable::~IsUserHome_Runnable(void) { this->stop(); }

void io::IsUserHome_Runnable::runnable(const boost::system::error_code &err,
    ...
    // error which is caught by IsUserHome_Runnable::thread_runner.
    ...
    t->async_wait(boost::bind(io::IsUserHome_Runnable::runnable,
                              boost::asio::placeholders::error, t, callback,
                              interval));
}

void io::IsUserHome_Runnable::thread_runner(void) {
    boost::asio::io_context io_context;
    boost::function<void(void)> callback(boost::bind(
        &io::IsUserHome_Runnable::isUserHome, this, HOME_DEVICE_IP));

    boost::asio::steady_timer timer(io_context, this->interval);
    timer.async_wait(boost::bind(io::IsUserHome_Runnable::runnable,
...
void io::IsUserHome_Runnable::start(void) {
    boost::function<void(void)> runner(
        boost::bind(&io::IsUserHome_Runnable::thread_runner, this));
    this->thread_group.create_thread(runner);
}

void io::IsUserHome_Runnable::stop(void) { this->thread_group.interrupt_all(); }

void io::IsUserHome_Runnable::attach(void) { this->thread_group.join_all(); }
