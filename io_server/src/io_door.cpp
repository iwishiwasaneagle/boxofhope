...
bool io::Door_Helper::door_state(void){
    const int debounceAvgCount = 5; //< The amount of times to sample the door before making a decision on the state.
    unsigned const int debounceDelay = 10; //< The time between samples. Increase if the output flickers.
...
void io::Door_Runnable::runnable(void) {
    ...
    while(1){        
        ...
        while((doorState = io::Door_Helper::door_state())==0){
           ...
           std::cout<<"\033[0;45;30mio::Door_Runnable\033[0m\tDoor open... waiting for it to be shut. "<<std::endl;
        ...
        try{
            ...
            std::cout<<"\033[0;45;30mio::Door_Runnable\033[0m\tTag found: "<<tagStr<<std::endl;
            uvcRunnable.start();
            uvcRunnable.attach();
        }catch(std::runtime_error err){
            std::cout<<"\033[0;45;30mio::Door_Runnable\033[0m\t No tag found..."<<std::endl;
            continue;
        }
    }
}
...
void io::Door_Runnable::stop(void) {
