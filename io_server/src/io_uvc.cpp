...
void io::UVC_Runnable::runnable(void) {
  ...
  // bundle of joy. In this block we ensure the door is closed (the first while
  // within the do-while), and then turn the LED on. After this, the door pin is
  // listened to (using poll!!!! Jan-Hendrik strengthened the WiringPi library
  // to essential create a DIY `wiringPiISR` here
  // https://github.com/WiringPi/WiringPi/pull/102 as this runnable is run in a
  // thread) for changes in the pin. If this change is because of the door
  // opening (the do-while within the do-while) then we turn off the LEDs.
  // Alternatively, if the io::Door_Helper::blocking_wait_for_door times out and
  // the remaining_time_ms is below 0, the LED's are alsoed turned off
  // (obviously).
  do {
    ...
    // Door closed and time remaining. We do this check so that if the blocking
    // poll call triggers for whatever reason, it resets itself.
    } while ((door_state = io::Door_Helper::door_state()) &&
             remaining_time_ms > 0);
    this->led(false);
...
void io::UVC_Runnable::stop(void) { this->thread_group.interrupt_all(); }
